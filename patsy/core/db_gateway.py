...
class DbGateway():
    ...
    def add(self, patsy_record: PatsyRecord) -> AddResult:
        ...
        location = self.find_or_create_location(patsy_record, storage_provider)
        if location:
    ...
    def find_or_create_storage_provider(self, patsy_record: PatsyRecord) -> Optional[Location]:
        ...
        storage_provider: StorageProvider = self.session.query(StorageProvider).filter(
            StorageProvider.storage_provider == patsy_record.storage_provider
        ).first()
    ...
    def find_or_create_location(self, patsy_record: PatsyRecord, storage_provider: StorageProvider) -> Optional[Location]:
